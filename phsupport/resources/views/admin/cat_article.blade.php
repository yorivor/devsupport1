    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                <div class="panel-heading">Knowledge Base Management -> Add Category -> Knowledge Base Article</div>
                <div class="panel-body">
                    <form class="form-horizontal" role="form" method="POST" action="{{ route('cat_article.post', $knowledge_id) }}" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        ...
                        <div class="form-group{{ $errors->has('knowledge_article_content') ? ' has-error' : '' }}">
                            <div style="padding:10px;">
                                ...
                                <br />
                                <textarea id="content knowledge_article_content" type="text" rows="20" class="form-control my-editor" name="knowledge_article_content" autofocus>{{ old('knowledge_article_content') }}</textarea>
                            </div>
                    ...
                    <div class="rows">
                        <table class="table table-striped">
                            ...
                            <?php 
                                $article = DB::table('knowledge_article')->where('knowledge_category_id', $knowledge_id)->orderBy('knowledge_article_id', 'desc')->paginate(10);
                                $count = count($article);
                            ?>
                            @if (! $count)
                            <tr>
                            ...
                            @else
                                @foreach($article as $articles)
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-info" href="{{ url('/edit_knowledge_article/'.$knowledge_id.'/0/'.$articles->knowledge_article_id) }}" data-toggle="tooltip" title="Edit Knowledge Base"><div class="glyphicon glyphicon-edit"></div></a> 
                                            <a  class="btn btn-danger" href="{{ url('/delete_knowledge/'.$knowledge_id.'/0/'.$articles->knowledge_article_id) }}" data-toggle="tooltip" title="Delete Knowledge Base"><div class="glyphicon glyphicon-trash"></div></a>
                                        </td>
                                    </tr>
                                @endforeach
                            @endif
