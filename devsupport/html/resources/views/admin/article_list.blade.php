    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                <div class="panel-heading">Article Management -> Add Downtime Advisory</div>
                <div class="panel-body">
                    <form class="form-horizontal" role="form" method="POST" action="{{ route('advirosry_add.post') }}" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        ...
                        <div class="form-group{{ $errors->has('article_content') ? ' has-error' : '' }}">
                            <div style="padding:10px;">
                                ...
                                <br />
                                <textarea id="content article_content" type="text" class="form-control my-editor" name="article_content" autofocus>{{ old('article_content') }}</textarea>
                            </div>
                    ...
                    <div class="rows">
                        <table class="table table-striped">
                            ...
                            <?php 
                                $article = DB::table('article')->where('article_category', 'Downtime Advisory')->orderBy('article_id', 'desc')->paginate(10);
                                $count = count($article);
                            ?>
                            @if (! $count)
                            <tr>
                            ...
                            @else
                                @foreach($article as $articles)
                                    <tr>
                                        ...
                                        <td>
                                            <a class="btn btn-info" href="{{ url('advirosry_edit/'.$articles->article_id) }}" data-toggle="tooltip" title="Edit Downtime Advisory"><div class="glyphicon glyphicon-edit"></div></a> 
                                            <a  class="btn btn-danger" href="{{ url('advirosry_delete/'.$articles->article_id) }}" data-toggle="tooltip" title="Delete Downtime Advisory"><div class="glyphicon glyphicon-trash"></div></a>
                                        </td>
                                    </tr>
                                @endforeach
                            @endif
